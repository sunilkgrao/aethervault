...
fn load_legacy_triggers_backup() -> Vec<TriggerEntry> {
    ...
    let bytes = match fs::read(&path) {
        ...
        Err(err) => {
            eprintln!("[load_triggers] failed to read trigger backup: {err}");
            return Vec::new();
        }
    };
    match serde_json::from_slice(&bytes) {
        Ok(triggers) => triggers,
        Err(err) => {
            eprintln!("[load_triggers] failed to parse legacy trigger backup: {err}");
            Vec::new()
        }
    }
}
...
pub(crate) fn restore_triggers_from_backup_if_empty(db: &MemoryDb) -> Result<usize, String> {
    let existing = db
        .triggers_list()
        .map_err(|err| format!("[trigger-restore] failed to query trigger count: {err}"))?;
    if !existing.is_empty() {
        return Ok(0);
    }

    let backup = load_legacy_triggers_backup();
    if backup.is_empty() {
        return Ok(0);
    }
