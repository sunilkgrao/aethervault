...
pub(crate) fn open_skill_db(path: &Path) -> Result<Connection, Box<dyn std::error::Error>> {
    ...
    if !cols.contains(&"times_succeeded".to_string()) {
        conn.execute_batch(
            "ALTER TABLE skills ADD COLUMN times_succeeded INTEGER NOT NULL DEFAULT 0",
        )?;
    }
...
fn search_skills_like(conn: &Connection, query: &str, limit: usize) -> Vec<SkillRecord> {
    ...
    let rows = match stmt.query_map(params![pattern, limit as i64], |row| Ok(row_to_skill(row))) {
        Ok(r) => r,
        Err(_) => return Vec::new(),
    };
...
pub(crate) fn prune_low_performing_skills(
    conn: &Connection,
    min_uses: u64,
    min_rate: f64,
) -> usize {
    ...
    let names: Vec<String> = stmt
        .query_map(params![min_uses as i64, min_rate], |row| row.get(0))
    ...
    if count > 0 {
        eprintln!(
            "[skill-prune] removed {count} low-performing skills (< {min_rate:.0}% after {min_uses}+ uses)"
        );
    }
    count
}
...
pub(crate) fn find_similar_skill(
    conn: &Connection,
    description: &str,
    threshold: f64,
) -> Option<String> {
    let new_words: HashSet<String> = description
        .split_whitespace()
    ...
    for skill in skills {
        if let Some(ref desc) = skill.description {
            let existing_words: HashSet<String> = desc
                .split_whitespace()
                .map(|w| w.to_lowercase())
                .filter(|w| w.len() > 2)
                .collect();
...
fn expand_synonyms(word: &str) -> Vec<String> {
    ...
    let synonyms: &[(&str, &[&str])] = &[
        ...
        (
            "implement",
            &["build", "code", "develop", "feature", "swarm"],
        ),
        ("feature", &["implement", "build", "develop", "swarm"]),
        (
            "develop",
            &["implement", "build", "code", "feature", "swarm"],
        ),
...
pub(crate) fn match_skills_for_prompt(
    ...
    let stop_words: HashSet<&str> = [
        ...
        "your", "i", "my", "me", "we", "our", "they", "their", "he", "she",
    ]
    .iter()
    .cloned()
    .collect();
...
pub(crate) fn bootstrap_skills(conn: &Connection) {
    ...
    eprintln!(
        "[bootstrap] seeded {} skills (bootstrap version {BOOTSTRAP_VERSION})",
        skills.len()
    );
}
