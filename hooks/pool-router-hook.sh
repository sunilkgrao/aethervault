#!/bin/bash
# AetherVault pool router — smart backend selection for subagents.
# Reads AgentHookRequest JSON from stdin, picks the best available
# backend (codex or claude-code), and delegates to it.

set -euo pipefail

PYTHON_HOOK="/root/.aethervault/hooks/pool-router-hook.py"
...
if [ $EXIT_CODE -gt 128 ]; then
    SIG=$((EXIT_CODE - 128))
    echo "{\"message\":{\"role\":\"assistant\",\"content\":\"(Pool router killed by signal $SIG)\",\"tool_calls\":[]}}"
fi
