...
LOCK_TIMEOUT = 30  # seconds to wait for file lock before giving up


def flock_with_timeout(fd, lock_type, timeout=LOCK_TIMEOUT):
    """Acquire flock with timeout. Raises TimeoutError if lock cannot be acquired."""
    ...
    while True:
        try:
            ...
            if time.monotonic() >= deadline:
                raise TimeoutError(
                    f"Could not acquire knowledge-graph lock within {timeout}s"
                )
            time.sleep(0.1)
...
def load_graph(graph_file=None):
    ...
    if os.path.exists(graph_file):
        ...
        with open(lock_path, "w") as lock_fd:
            flock_with_timeout(lock_fd, fcntl.LOCK_SH)
            try:
...
def save_graph(G, graph_file=None):
    ...
    lock_path = graph_file + ".lock"
    with open(lock_path, "w") as lock_fd:
        flock_with_timeout(lock_fd, fcntl.LOCK_EX)
        try:
