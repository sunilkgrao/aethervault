#!/bin/bash
# AetherVault model_hook wrapper for Claude Code CLI
# Reads AgentHookRequest JSON from stdin, passes to Python hook.
#
# NO hard timeout. Claude Code tasks can run for extended periods.
...
PYTHON_HOOK="/root/.aethervault/hooks/claude-code-model-hook.py"
...
if [ $EXIT_CODE -gt 128 ]; then
    SIG=$((EXIT_CODE - 128))
    echo "{\"message\":{\"role\":\"assistant\",\"content\":\"(Claude Code hook killed by signal $SIG)\",\"tool_calls\":[]}}"
fi
