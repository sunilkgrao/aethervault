...
# Timeout for aethervault binary operations (seconds)
AV_TIMEOUT=120  # 2 minutes for doctor/compact
AV_MERGE_TIMEOUT=300  # 5 minutes for merge/rebuild

# Wrapper: run aethervault binary with timeout to prevent hangs
av_run() {
    timeout "$AV_TIMEOUT" "$AV" "$@"
}

av_run_long() {
    timeout "$AV_MERGE_TIMEOUT" "$AV" "$@"
}
...
get_effective_limit() {
    local capsule="${1:-$MV2}"
    local doctor_out
    doctor_out=$(av_run doctor --dry-run "$capsule" 2>&1)
    local wal_size
...
get_data_size() {
    local status_out
    status_out=$(av_run doctor --dry-run "$1" 2>&1)
    local footer_offset
...
get_wal_size() {
    local doctor_out
    doctor_out=$(av_run doctor --dry-run "$1" 2>&1)
    echo "$doctor_out" | grep -oP 'wal_size=\K[0-9]+' || echo "0"
}

get_frame_count() {
    av_run doctor --dry-run "$1" 2>&1 | grep -oP 'frames=\K[0-9]+' || \
    av_run status "$1" 2>&1 | grep "frames:" | awk '{print $2}' || echo "0"
}

get_collection_count() {
    local capsule="$1" collection="$2"
    local result
    result=$(av_run search "$capsule" "the" --collection "$collection" --limit 9999 2>/dev/null | grep -c "aethervault://$collection" 2>/dev/null) || result=0
    echo "${result:-0}"
}
...
send_telegram_alert() {
    ...
    curl -s --connect-timeout 10 --max-time 30 -X POST "https://api.telegram.org/bot${bot_token}/sendMessage" \
...
cmd_verify() {
    ...
    local result
    result=$(av_run doctor --dry-run "$MV2" 2>&1)
    echo "$result"
...
cmd_compact() {
    ...
    local verify
    verify=$(av_run doctor --dry-run "$MV2" 2>&1)
...
cmd_archive() {
    ...
    av_run compact "$archive_path" 2>&1 | grep -E "status|duration" || true

    # Verify archive integrity
    local verify
    verify=$(av_run doctor --dry-run "$archive_path" 2>&1)
    if echo "$verify" | grep -q "Clean"; then
...
cmd_rebuild() {
    ...
    if av_run config "$MV2" get --key index --raw > "$config_tmp" 2>/dev/null && [ -s "$config_tmp" ]; then
    ...
    av_run init "$tmp_empty" 2>/dev/null

    log "Merging capsule (dedup + fresh WAL)..."
    local merge_stderr="/tmp/rebuild-merge-stderr-$$.log"
    if ! av_run_long merge "$MV2" "$tmp_empty" "$tmp_rebuilt" --json 2>"$merge_stderr"; then
        local err_msg
    ...
    if [ -s "$config_tmp" ] && [ "$(cat "$config_tmp")" != "{}" ]; then
        av_run config "$tmp_rebuilt" set --key index --file "$config_tmp" 2>/dev/null
    ...
    # Verify new capsule
    local verify
    verify=$(av_run doctor --dry-run "$tmp_rebuilt" 2>&1)
    ...
    new_wal=$(av_run doctor --dry-run "$tmp_rebuilt" 2>&1 | grep -oP 'wal_size=\K[0-9]+' || echo "0")
...
cmd_watchdog() {
    ...
    if [ ! -f "$MV2" ]; then
        ...
        send_telegram_alert "CAPSULE MISSING: $MV2 not found on clawdbot server"
        exit 2
    fi
    ...
    if [ "$data_size" -gt "$crit_threshold" ]; then
        ...
        av_run config "$MV2" get --key index --raw > "$config_tmp" 2>/dev/null || echo "{}" > "$config_tmp"
        ...
        av_run init "$tmp_empty" 2>/dev/null
        if av_run_long merge "$MV2" "$tmp_empty" "$tmp_rebuilt" 2>/dev/null; then
    ...
    if [ "$disk_pct" -ge 90 ]; then
        ...
        send_telegram_alert "DISK CRITICAL: ${disk_pct}% full on clawdbot server. Free space needed."
