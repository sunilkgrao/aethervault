...
use aether_core::{Vault, PutOptions, Result, SearchRequest};
...
fn main() -> Result<()> {
    ...
    println!("=== Vault PDF Ingestion Example ===\n");

    // ========================================
    // 1. CREATE a new memory file
    // ========================================
    println!("1. Creating memory file...");
    let mut mem = Vault::create(&mv2_path)?;
    ...
    drop(mem);
    let report = Vault::verify(&mv2_path, false)?;
