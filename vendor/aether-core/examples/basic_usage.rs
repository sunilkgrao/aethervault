...
use aether_core::{Vault, PutOptions, Result, SearchRequest, TimelineQuery};
...
fn main() -> Result<()> {
    ...
    println!("=== Vault Core Basic Usage Example ===\n");
    ...
    let seq1 = mem.put_bytes(b"Hello, Vault! This is a simple text document.")?;
    ...
    let seq2 = mem.put_bytes_with_options(
        b"This guide covers the basics of using Vault for AI memory storage.",
        options,
    )?;
    ...
    mem.put_bytes_with_options(
        b"The Vault API provides methods for create, put, find, and timeline operations.",
        options,
    )?;
    ...
    let options = PutOptions::builder()
        ...
        .build();
    mem.put_bytes_with_options(
        b"Frequently asked questions about Vault memory files and search.",
        options,
    )?;
    ...
    let report = Vault::verify(&path, false)?;
