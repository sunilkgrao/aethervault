...
use crate::{FrameId, Vault, Result};
    ...
    fn match_triple(&self, triple: &TriplePattern) -> Vec<GraphMatchResult> {
        ...
        match (&triple.subject, &triple.predicate, &triple.object) {
            ...
            (
                ...
                for entity in self.vault.memory_entities() {
                    let cards = self.vault.get_entity_memories(&entity);
                    for card in cards {
            ...
            (
                ...
                if let Some(card) = self.vault.get_current_memory(entity, slot) {
                    let mut result =
...
pub fn hybrid_search(vault: &mut Vault, plan: &QueryPlan) -> Result<Vec<HybridSearchHit>> {
    match plan {
        QueryPlan::VectorOnly {
            ...
            let response = vault.search(request)?;
            Ok(response
                .hits
                .iter()
                .map(|h| {
        ...
        QueryPlan::GraphOnly { pattern, limit } => {
            let matcher = GraphMatcher::new(vault);
        ...
        QueryPlan::Hybrid {
            ...
            for &frame_id in &candidate_frames {
                ...
                let preview = vault.frame_preview_by_id(frame_id).ok();
