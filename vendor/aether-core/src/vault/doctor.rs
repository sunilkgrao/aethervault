...
use crate::vault::lifecycle::{Vault, ensure_single_file, read_toc, recover_toc};
use crate::types::{
...
fn try_recover_from_wal_corruption(path: &Path) -> Result<Vault> {
    use fs2::FileExt;
...
impl DoctorExecutor {
    ...
    fn run(self) -> Result<DoctorReport> {
        ...
        for phase in &plan.phases {
            ...
            if matches!(phase.phase, DoctorPhaseKind::Verify) {
                if !overall_failed {
                    ...
                    {
                        if let Ok(mut reopen) = Vault::try_open(&path) {
        ...
        if overall_failed {
            if let Some(original) = &original_header {
                if let Ok(mut revert) = Vault::try_open(&path) {
    ...
    fn reset_wal(mem: &mut Vault) -> Result<()> {
        doctor_log!(
    ...
    fn is_toc_corruption_error(err: &VaultError) -> bool {
        matches!(
            err,
            VaultError::InvalidToc { .. } | VaultError::InvalidHeader { .. }
        )
    }
}
