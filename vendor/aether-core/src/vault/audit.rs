...
//!
//! This module provides the `audit` method on `Vault` that generates
...
impl Vault {
    ...
    ///
    /// ```ignore
    /// let report = mem.audit("What is Vault?", None, None::<&dyn VecEmbedder>)?;
...
impl Vault {
    ...
    where
        E: VecEmbedder + ?Sized,
    {
        Err(crate::VaultError::LexNotEnabled)
    }
...
mod tests {
    ...
    fn test_audit_basic() {
        run_serial_test(|| {
            ...
            mem.put_bytes_with_options(
                b"Vault is a single-file AI memory system that provides instant retrieval.",
                options,
            )
            .expect("put");
            ...
            let report = mem
                .audit::<dyn VecEmbedder>("What is Vault?", None, None)
                .expect("audit");

            assert_eq!(report.version, "1.0");
            assert_eq!(report.question, "What is Vault?");
            assert!(!report.sources.is_empty());

            let source = &report.sources[0];
            assert_eq!(source.uri, "mv2://docs/intro.md");
            assert_eq!(source.title.as_deref(), Some("Introduction to Vault"));
    ...
    fn test_audit_with_snippets() {
        run_serial_test(|| {
            ...
            mem.put_bytes(b"Vault provides fast semantic search capabilities.")
                .expect("put");
            mem.commit().expect("commit");
