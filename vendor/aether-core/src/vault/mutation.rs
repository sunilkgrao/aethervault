//! Frame mutation and ingestion routines for `Vault`.
//!
...
use crate::vault::chunks::{plan_document_chunks, plan_text_chunks};
use crate::vault::lifecycle::{Vault, prepare_toc_bytes};
use crate::reader::{
...
use crate::{
    DEFAULT_SEARCH_TEXT_LIMIT, ExtractedDocument, VaultError, Result, TimeIndexEntry,
...
fn log_reader_result(reader: &str, diagnostics: &ReaderDiagnostics, elapsed: Duration) {
    ...
    if warnings > 0 || diagnostics.fallback {
        ...
        for warning in &diagnostics.warnings {
            tracing::warn!(target = "vault::extract", reader, warning = %warning);
        }
    } else {
        tracing::info!(
            target = "vault::extract",
...
impl Vault {
    ...
    fn append_wal_entry(&mut self, payload: &[u8]) -> Result<u64> {
        loop {
            match self.wal.append_entry(payload) {
                ...
                Err(VaultError::CheckpointFailed { reason })
    ...
    fn grow_wal_region(&mut self, required_entry_size: u64) -> Result<()> {
        ...
        while new_size <= target {
            new_size = new_size
                .checked_mul(2)
                .ok_or_else(|| VaultError::CheckpointFailed {
            ...
            if used_parallel {
                ...
                if self.lex_enabled {
                    ...
                    if tantivy_was_present {
                        ...
                        let max_payload = crate::vault::search::max_index_payload();
                        ...
                        for frame_id in &delta.inserted_frames {
                            ...
                            if !crate::vault::search::is_text_indexable_mime(mime) {
                                continue;
                            }
            ...
            for record in records {
                ...
                match entry.op {
                    ...
                    FrameWalOp::Tombstone => {
                        let target = entry.target_frame_id.ok_or(VaultError::InvalidFrame {
                            frame_id: 0,
    ...
    fn collect_temporal_mentions(
        ...
        let regex = match regex {
            Ok(re) => re,
            Err(msg) => {
                tracing::error!(target = "vault::temporal", error = %msg, "numeric date regex init failed");
                return Vec::new();
            }
        };
    ...
    fn mark_frame_superseded(&mut self, frame_id: FrameId, successor_id: FrameId) -> Result<()> {
        let index = usize::try_from(frame_id).map_err(|_| VaultError::InvalidFrame {
            frame_id,
        ...
        if self.lex_enabled {
            if let Some(ref engine) = self.tantivy {
                ...
                let text_indexable_count = self
                    ...
                    .frames
                    .iter()
                    .filter(|f| crate::vault::search::is_frame_text_indexable(f))
                    .count();
        ...
        match self.tier() {
            ...
            tier => {
                if self.toc.ticket_ref.issuer.trim().is_empty() {
                    Err(VaultError::TicketRequired { tier })
                } else {
                    Ok(())
                }
            }
