...
use crate::{LexSearchHit, VaultError, Result, VecSearchHit};
...
impl Vault {
    pub fn enable_lex(&mut self) -> Result<()> {
        ...
        if self.lex_enabled {
            ...
            #[cfg(feature = "lex")]
            if self.lex_index.is_none() && crate::vault::lifecycle::has_lex_index(&self.toc) {
    ...
    pub fn search_lex(&mut self, query: &str, limit: usize) -> Result<Vec<LexSearchHit>> {
        if !self.lex_enabled {
            return Err(VaultError::LexNotEnabled);
        }
        self.ensure_lex_index()?;
        let index = self.lex_index.as_ref().ok_or(VaultError::LexNotEnabled)?;
    ...
    pub fn search_vec(&mut self, query: &[f32], limit: usize) -> Result<Vec<VecSearchHit>> {
        if !self.vec_enabled {
            return Err(VaultError::VecNotEnabled);
        }
        ...
        let index = self.vec_index.as_ref().ok_or(VaultError::VecNotEnabled)?;
        Ok(index.search(query, limit))
    }
    ...
    pub fn add_clip_embedding_with_page(
        ...
        if !self.clip_enabled {
            return Err(VaultError::ClipNotEnabled);
        }
    ...
    pub fn search_clip(
        ...
        let index = self
            .clip_index
            .as_ref()
            .ok_or(VaultError::ClipNotEnabled)?;
    ...
    pub fn vec_search_with_embedding(
        ...
        let vec_index = self.vec_index.as_ref().ok_or(VaultError::VecNotEnabled)?;
    ...
    /// let result = vault.search_adaptive("query", &embedding, config, 200, None)?;
    ...
    pub fn timeline(&mut self, query: TimelineQuery) -> Result<Vec<TimelineEntry>> {
        ...
        #[cfg(feature = "temporal_track")]
        {
            crate::vault::timeline::build_timeline(
                self,
                limit,
                since,
                until,
        ...
        {
            crate::vault::timeline::build_timeline(self, limit, since, until, reverse)
        }
    }
}
...
impl Vault {
    ...
    fn materialize_tantivy_segments(&mut self, segments: &[EmbeddedLexSegment]) -> Result<TempDir> {
        ...
        let mut file_len =
            self.file
                ...
                .map_err(|err| VaultError::Tantivy {
                    reason: format!("failed to inspect vault file metadata: {err}"),
                })?;
        ...
        for segment in segments {
            ...
            if let Some(parent) = dest.parent() {
                fs::create_dir_all(parent).map_err(|err| VaultError::Tantivy {
                    reason: format!(
            ...
            let mut writer = File::create(&dest).map_err(|err| VaultError::Tantivy {
                reason: format!(
            ...
            if end > file_len || end > data_limit {
                if self.align_footer_with_catalog()? {
                    file_len = self.file.metadata().map(|meta| meta.len()).map_err(|err| {
                        VaultError::Tantivy {
                            reason: format!("failed to refresh vault file metadata: {err}"),
                        }
                    })?;
    ...
    pub(crate) fn init_tantivy(&mut self) -> Result<()> {
        ...
        let expected_docs = self
            ...
            .read()
            .ok()
            .and_then(|storage| crate::vault::lifecycle::lex_doc_count(&self.toc, &storage));
...
/// Can be overridden via `AETHERVAULT_MAX_INDEX_PAYLOAD` environment variable
...
pub fn max_index_payload() -> u64 {
    std::env::var("AETHERVAULT_MAX_INDEX_PAYLOAD")
        .ok()
...
impl Vault {
    pub(crate) fn rebuild_tantivy_engine(&mut self, engine: &mut TantivyEngine) -> Result<bool> {
        ...
        for frame in active_frames {
            ...
            if frame.payload_length > max_payload {
                tracing::debug!(
                    "skipping frame {} - payload {} exceeds max indexable size {} (AETHERVAULT_MAX_INDEX_PAYLOAD)",
                    frame.id,
