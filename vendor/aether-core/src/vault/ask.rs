...
use crate::vault::search::helpers::{build_context, reorder_hits_by_token_matches};
...
use crate::{VaultError, Result, VecEmbedder};
...
fn vector_hits(
    ...
    if !vault.vec_enabled || query_embedding.is_empty() {
        return Ok(Vec::new());
    }
    ...
    if let Some(ref adaptive_config) = request.adaptive {
        if adaptive_config.enabled {
            let result = vault.search_adaptive(
                &request.question,
                query_embedding,
    ...
    let vec_response = vault.vec_search_with_embedding(
        &request.question,
        query_embedding,
        limit,
        request.snippet_chars,
...
fn promote_corrections(vault: &mut Vault, hits: &mut Vec<SearchHit>) -> Result<()> {
    if hits.is_empty() {
        return Ok(());
    }
    ...
    for (idx, hit) in hits.iter().enumerate() {
        ...
        if hit.uri.contains("mv2://correction/") {
            if let Ok(frame) = vault.frame_by_id(hit.frame_id) {
                let boost = frame
                    .extra_metadata
                    .get("aethervault.correction.boost")
...
fn promote_temporal_extremes(
    ...
    use crate::vault::search::parse_content_date_to_timestamp;
