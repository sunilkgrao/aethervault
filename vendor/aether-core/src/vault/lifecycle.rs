...
impl Vault {
    ...
    pub fn create<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        #[cfg(feature = "parallel_segments")]
        vault.load_manifest_segments(manifest_wal_entries);

        vault.bootstrap_segment_catalog();

        // Create empty manifests for enabled indexes so they persist across open/close
        let empty_offset = vault.data_end;
        let empty_checksum = *b"\xe3\xb0\xc4\x42\x98\xfc\x1c\x14\x9a\xfb\xf4\xc8\x99\x6f\xb9\x24\
                                \x27\xae\x41\xe4\x64\x9b\x93\x4c\xa4\x95\x99\x1b\x78\x52\xb8\x55";

        #[cfg(feature = "lex")]
        if vault.lex_enabled && vault.toc.indexes.lex.is_none() {
            vault.toc.indexes.lex = Some(crate::types::LexIndexManifest {
                doc_count: 0,
                generation: 0,
        ...
        #[cfg(feature = "vec")]
        if vault.vec_enabled && vault.toc.indexes.vec.is_none() {
            vault.toc.indexes.vec = Some(crate::types::VecIndexManifest {
                vector_count: 0,
                dimension: 0,
                bytes_offset: empty_offset,
                bytes_length: 0,
        ...
        vault.rewrite_toc_footer()?;
        vault.header.toc_checksum = vault.toc.toc_checksum;
        crate::persist_header(&mut vault.file, &vault.header)?;
        vault.file.sync_all()?;
        Ok(vault)
    }
    ...
    fn open_locked(mut file: File, lock: FileLock, path_ref: &Path) -> Result<Self> {
        ...
        if is_mv2e {
            return Err(VaultError::EncryptedFile {
                path: path_ref.to_path_buf(),
                hint: format!("Run: vault unlock {}", path_ref.display()),
            });
        }
        ...
        let toc = match read_toc(&mut file, &header) {
            Ok(toc) => toc,
            Err(err @ (VaultError::Decode(_) | VaultError::InvalidToc { .. })) => {
        ...
        vault.data_end = compute_data_end(&vault.toc, &vault.header);
        // Use consolidated helper for lex_enabled check
        vault.lex_enabled = has_lex_index(&vault.toc);
        if vault.lex_enabled {
            vault.load_lex_index_from_manifest()?;
        }
        #[cfg(feature = "lex")]
        {
            vault.init_tantivy()?;
        ...
        vault.vec_enabled =
            vault.toc.indexes.vec.is_some() || !vault.toc.segment_catalog.vec_segments.is_empty();
        if vault.vec_enabled {
            vault.load_vec_index_from_manifest()?;
        }
        vault.clip_enabled = vault.toc.indexes.clip.is_some();
        if vault.clip_enabled {
            vault.load_clip_index_from_manifest()?;
        }
        vault.recover_wal()?;
        #[cfg(feature = "parallel_segments")]
        vault.load_manifest_segments(manifest_wal_entries);
        vault.bootstrap_segment_catalog();
        #[cfg(feature = "temporal_track")]
        vault.ensure_temporal_track_loaded()?;
        vault.load_memories_track()?;
        vault.load_logic_mesh()?;
        vault.load_sketch_track()?;
        if checksum_result.is_err() {
            vault.toc.verify_checksum()?;
            if vault.toc.toc_checksum != vault.header.toc_checksum {
                vault.header.toc_checksum = vault.toc.toc_checksum;
                crate::persist_header(&mut vault.file, &vault.header)?;
                vault.file.sync_all()?;
            }
    ...
    pub fn bind_memory(
        ...
        if let Some(existing) = self.get_memory_binding() {
            if existing.memory_id != binding.memory_id {
                return Err(VaultError::MemoryAlreadyBound {
...
pub(crate) fn read_toc(file: &mut File, header: &Header) -> Result<Toc> {
    ...
    let footer = CommitFooter::decode(footer_bytes).ok_or(VaultError::InvalidToc {
...
fn verify_toc_prefix(bytes: &[u8]) -> Result<()> {
    ...
    let read_u64 = |range: std::ops::Range<usize>, context: &str| -> Result<u64> {
        let slice = bytes.get(range).ok_or_else(|| VaultError::InvalidToc {
            reason: context.to_string().into(),
        })?;
        let array: [u8; 8] = slice.try_into().map_err(|_| VaultError::InvalidToc {
            reason: context.to_string().into(),
        })?;
...
fn ensure_non_overlapping_frames(toc: &Toc, file_len: u64) -> Result<()> {
    ...
    for frame in frames_by_offset {
        let end = frame
            ...
            .ok_or_else(|| VaultError::InvalidToc {
...
fn load_tail_snapshot(file: &File) -> Result<TailSnapshot> {
    ...
    let (slice, offset_adjustment) =
        locate_footer_window(&mmap).ok_or_else(|| VaultError::InvalidToc {
...
pub(crate) fn ensure_single_file(path: &Path) -> Result<()> {
    if let Some(parent) = path.parent() {
        ...
        for suffix in forbidden {
            ...
            if candidate.exists() {
                return Err(VaultError::AuxiliaryFileDetected { path: candidate });
            }
        }
...
mod tests {
    ...
    fn ensure_single_file_blocks_sidecars() {
        ...
        assert!(matches!(
            result,
            Err(VaultError::AuxiliaryFileDetected { .. })
        ));
    }
}
