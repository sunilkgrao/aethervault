...
/// The AetherVault core crate version (matches `Cargo.toml`).
pub const AETHERVAULT_CORE_VERSION: &str = env!("CARGO_PKG_VERSION");
...
pub use vault::{
    BlobReader, EnrichmentHandle, EnrichmentStats, LockSettings, OpenReadOptions, SketchCandidate,
    SketchSearchOptions, SketchSearchStats, Vault,
    mutation::{CommitMode, CommitOptions},
    start_enrichment_worker, start_enrichment_worker_with_embeddings,
};
#[cfg(feature = "parallel_segments")]
pub use vault::{BuildOpts, ParallelInput, ParallelPayload};
pub use models::{
...
pub use types::{
    ...
    AETHERVAULT_EMBEDDING_DIMENSION_KEY, AETHERVAULT_EMBEDDING_MODEL_KEY, AETHERVAULT_EMBEDDING_NORMALIZED_KEY,
    AETHERVAULT_EMBEDDING_PROVIDER_KEY, MediaManifest, VaultHandle, Open, PutOptions,
...
pub use vault::memory::SchemaSummaryEntry;
...
pub(crate) fn run_serial_test<T>(f: impl FnOnce() -> T) -> T {
    let _guard = SERIAL_TEST_MUTEX
        .lock()
        .expect("vault-core serial test mutex poisoned");
    f()
}
...
impl Drop for Vault {
    fn drop(&mut self) {
        ...
        #[cfg(feature = "parallel_segments")]
        {
            use crate::vault::lifecycle::cleanup_manifest_wal_public;
...
pub(crate) fn decode_canonical_bytes(
    ...
    match encoding {
        ...
        CanonicalEncoding::Zstd => {
            zstd::decode_all(Cursor::new(payload)).map_err(|_| VaultError::InvalidFrame {
                frame_id,
...
mod tests {
    ...
    fn create_put_commit_reopen() {
        run_serial_test(|| {
            ...
            let mut reopened = Vault::open(&path).expect("open");
    ...
    fn verify_reports_success() {
        run_serial_test(|| {
            ...
            let report = Vault::verify(&path, true).expect("verify");
    ...
    fn test_create_enables_indexes_by_default() {
        run_serial_test(|| {
            ...
            let mem = Vault::create(&path).expect("create");
    ...
    fn doctor_rebuilds_time_index() {
        ...
        run_serial_test(|| {
            ...
            let reopened = Vault::open(&path).expect("reopen after doctor");
            assert!(
    ...
    fn ticket_sequence_enforced() {
        run_serial_test(|| {
            ...
            assert!(matches!(err, VaultError::TicketSequence { .. }));
        });
    }
