...
use aether_core::{
    DoctorOptions, DoctorPhaseKind, DoctorStatus, HEADER_SIZE, Vault, PutOptions, SearchRequest,
...
fn doctor_recovers_corrupted_wal() {
    use aether_core::io::header::HeaderCodec;
    use std::fs::{read, write};
    ...
    let report = Vault::doctor(&mv2_path, options).expect("docter");
    eprintln!("Doctor status: {:?}", report.status);
    ...
    let _mem = Vault::open(&mv2_path).expect("should open after repair");
}
...
fn doctor_repairs_header_pointer() {
    ...
    let report = Vault::doctor(&mv2_path, options).expect("doctor report");
    ...
    let _ = Vault::open(&mv2_path).expect("open");
}
...
fn doctor_recovers_corrupted_toc() {
    ...
    let _ = Vault::open(&mv2_path).expect("file should open after repair");
}
...
fn doctor_handles_missing_footer() {
    ...
    {
        let mut mem = Vault::create(&mv2_path).expect("create mv2");
    ...
    let report = Vault::doctor(&mv2_path, options).unwrap();
...
fn doctor_dry_run_returns_plan() {
    ...
    {
        let mut mem = Vault::create(&mv2_path).expect("create mem");
    ...
    let report = Vault::doctor(&mv2_path, options).expect("report");
    assert!(matches!(
        report.status,
...
fn doctor_rejects_index_out_of_bounds() {
    ...
    let report = Vault::doctor(&mv2_path, options).expect("doctor");
    assert!(
...
fn doctor_vacuum_before_index_rebuild() {
    ...
    {
        let mut mem = Vault::open(&mv2_path).expect("open");
...
fn doctor_preserves_footer_offset() {
    ...
    let _ = Vault::doctor(&mv2_path, options).expect("doctor");
    ...
    let _ = Vault::open(&mv2_path).expect("file should open after doctor");
}
